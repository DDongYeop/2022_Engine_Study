...
public class StageController : MonoBehaviour
{
	...
	[SerializeField]
	private	UIController			uiController;							// 게임오버 되었을 때 UI 활성화
	...
	/// <summary>
	/// 블록 배치 후처리 코루틴 메소드 호출
	/// </summary>
	public void AfterBlockArrangement(DragBlock block)
	{
		// 블록 배치 완료 후 행동 처리 (블록 삭제, 점수 계산, 드래그 블록 생성 등)
	...
	private bool IsGameOver()
	{
		...
		for ( int i = 0; i < dragBlockSpawner.BlockSpawnPoints.Length; ++ i )
		{
			...
			if ( dragBlockSpawner.BlockSpawnPoints[i].childCount != 0 )
			{
				...
				// IsPossibleArrangement()가 true를 반환하면 블록을 배치할 수 있다는 뜻으로 IsGameOver()는 false를 반환해 게임 진행
