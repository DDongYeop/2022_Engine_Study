...
public class Board : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		_blockSize = (1080 - 85 - 25 * (BlockCount.x-1)) / BlockCount.x;

		_currentScore = 0;
		uiController.UpdateCurrentScore(_currentScore);
		_highScore = PlayerPrefs.GetInt("HighScore");
		uiController.UpdateHighScore(_highScore);

		NodeList = nodeSpawner.SpawnNodes(this, BlockCount, _blockSize);
	...
	{
		if ( NodeList[y*BlockCount.x+x].placedBlock != null ) 
			return;
		...
		clone.GetComponent<RectTransform>().sizeDelta = new Vector2(_blockSize, _blockSize);
	...
	private void UpdateState()
	{
		...
		if ( targetAllNull && _state == State.Processing )
		{
